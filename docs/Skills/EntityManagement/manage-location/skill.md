...
Follow [article-location](../../Kempopedia/article-location/skill.md) to write the article.
...
| shortCode | Optional | ISO-style code (US, JP, DE) |
| dateFounded | Optional | Founding/independence date |
| dateDissolved | Optional | For historical nations — null if active |
| lat, long | Recommended | Coordinates — required for map visibility |
| articleId | **Required*** | Link to article |

**State fields:**
| Field | Requirement | Description |
|-------|-------------|-------------|
| name | **Required** | State name |
| abbreviation | Optional | Two-letter code (MI, NY) |
| stateType | Has default | "state", "territory", "province" — defaults to "state" |
| nationId | **Required** | Parent nation |
| dateFounded | Optional | When state was established/admitted |
| dateDisbanded | Optional | For historical states — null if active |
| lat, long | Recommended | Coordinates — required for map visibility |
| articleId | **Required*** | Link to article |
...
| cityType | Has default | "city", "town", "village" — defaults to "city" |
| stateId | **Required** | Parent state |
| dateFounded | Optional | When city was founded/incorporated |
| dateDisbanded | Optional | For historical cities — null if active |
| lat, long | Recommended | Coordinates — required for map visibility |
| articleId | **Required*** | Link to article |
...
| placeType | Recommended | building, venue, landmark, park, etc. |
| cityId | **Required** | Parent city |
| parentPlaceId | Optional | Parent place — for nested places only |
| address | Optional | Street address |
| dateOpened | Optional | Opening date |
| dateClosed | Optional | Closing date — null if active |
| lat, long | Recommended | Coordinates — for significant places |
| articleId | Recommended | Link to article — not all minor places need articles |

*Every Nation, State, and City record should have a linked article. Create the article first, then link it.

### Step 6: Set Coordinates

All cities and significant places should have coordinates for map visibility at `/admin/world-data/locations/map`.

**Finding coordinates:**
1. Identify the real-world inspiration location
2. Look up its approximate coordinates (e.g., via Google Maps)
3. Use similar but not identical coordinates for the Kempo location

**Examples:**
| Kempo Location | Inspiration | Coordinates |
|----------------|-------------|-------------|
| Motor City | Detroit (42.3°N, 83.0°W) | 42.4°N, 83.1°W |
| Steel City | Pittsburgh (40.4°N, 80.0°W) | 40.5°N, 79.9°W |
| Borjali | Gori, Georgia (42.0°N, 44.1°E) | 42.1°N, 44.5°E |
| Pine Ridge, VA | Hot Springs, VA (38.0°N, 79.8°W) | 38.2°N, 79.5°W |

### Step 7: Link the Article

In the location form, select the article from the dropdown.

### Step 8: Link the Image
...
- [ ] **Coordinates set** (lat/long for map visibility)
