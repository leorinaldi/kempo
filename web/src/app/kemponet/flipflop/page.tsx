...
export default function FlipFlopPage() {
  ...
  // Store initial video ID from URL
  const initialVideoIdRef = useRef<string | null>(null)

  // Detect mobile/kemponet context and initial video
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        // If there's an initial video ID, start with that video
        if (initialVideoIdRef.current) {
          ...
          initialVideoIdRef.current = null // Clear so it doesn't re-trigger
        }
      })
      .catch((err) => {
